...
===============================================================================
 dec2base76.c
 Takes a decimal input and returns Base76 (or reverse with -r)
 ...
 Usage:
   ./dec2base76 <decimal_value>
   ./dec2base76 -r <base76_value>  // reverse: Base76 → decimal
===============================================================================
*/
...
void validate_decimal(const char *s) {
    if (*s == '\0') {
    ...
    for (const char *p = s; *p; p++) {
        if (!isdigit((unsigned char)*p)) {
            fprintf(stderr, "Error: Invalid decimal digit '%c'.\n", *p);
            exit(1);
        }
    }
}
